...
def load_picture():
    ...
    for i in range(20):
        a=len(os.listdir("F:\\360MoveData\\Users\\1\\Desktop\\zhinengzhizao\\cv\\cv-img\\picture_train\\"+str(i)))
        for j in range(a):
            img=cv.imread("F:\\360MoveData\\Users\\1\\Desktop\\zhinengzhizao\\cv\\cv-img\\picture_train\\"+str(i)+"/"+str(j)+".tiff", 0)
            train_data[train_index] = img
            train_label[train_index] = i
            train_index += 1

        for k in range(4):
            img=cv.imread("F:\\360MoveData\\Users\\1\\Desktop\\zhinengzhizao\\cv\\cv-img\\picture_test/"+str(i)+"/"+str(k)+".tiff", 0)
...
#保存模型
f=open('F:\\360MoveData\\Users\\1\\Desktop\\zhinengzhizao\\cv\\cv-img\\model\\clf_svm.pkl',"wb")
pickle.dump(clf,f)
f.close()
